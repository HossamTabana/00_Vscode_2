...
SHOULD_BUILD_BASE="$(grep -m 1 build_base build.yml | awk -F\" '{print $(NF-1)}')"
SHOULD_BUILD_SPARK="$(grep -m 1 build_spark build.yml | awk -F\" '{print $(NF-1)}')"
SHOULD_BUILD_JUPYTERLAB="$(grep -m 1 build_jupyter build.yml | awk -F\" '{print $(NF-1)}')"

SPARK_VERSION="$(grep -m 1 spark build.yml | awk -F\" '{print $(NF-1)}')"
JUPYTERLAB_VERSION="$(grep -m 1 jupyterlab build.yml | awk -F\" '{print $(NF-1)}')"
