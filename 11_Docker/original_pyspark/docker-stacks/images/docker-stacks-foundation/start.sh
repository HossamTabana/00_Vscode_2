...
# the ibrahho user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the ibrahho user to the desired the user (NB_USER)
    if id ibrahho &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" ibrahho 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the ibrahho user:"
            _log "- username: ibrahho       -> ${NB_USER}"
            _log "- home dir: /home/ibrahho -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the ibrahho user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the ibrahho home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "ibrahho" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/ibrahho to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/ibrahho/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/ibrahho to /home/${NB_USER}!"
                _log "Attempting to symlink /home/ibrahho to /home/${NB_USER}..."
                if ln -s /home/ibrahho "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/ibrahho to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/ibrahho/"* ]]; then
...
# user we started as.
    ...
    IBRAHHO_UID="$(id -u ibrahho 2>/dev/null)"  # The default UID for the ibrahho user
    IBRAHHO_GID="$(id -g ibrahho 2>/dev/null)"  # The default GID for the ibrahho user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old ibrahho user to nayvoj ($(id -u ibrahho):$(id -g ibrahho))"
            ...
            sed --expression="s/^ibrahho:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/ibrahho:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "ibrahho" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/ibrahho. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/ibrahho ]]; then
        _log "WARNING: no write access to /home/ibrahho. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
